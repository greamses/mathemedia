let currentProblem = {};
let selectedTable = null; // No table selected initially
let score = 0;
...
function handleJump(e) {
  ...
  if (gameOver) {
    resetGame();
  } else if (selectedTable !== null) { // Only jump if table is selected
    bird.velocity = jump;
...
function handleChoice(choice) {
  ...
  if (choice === currentProblem.answer) {
    ...
    updateScore();
    generateMultiplicationProblem(); // Changed to generate addition problem
  } else {
...
function generateMultiplicationProblem() {
  if (selectedTable === null) {
    return; // Do not proceed if no table is selected
  }

  const multiplier = Math.floor(Math.random() * 12) + 1; // Random number between 1 and 12
  const correctAnswer = selectedTable * multiplier;

  // Display the multiplication problem
  mathProblemElement.textContent = `${selectedTable} x ${multiplier}`;
  currentProblem = { answer: correctAnswer };

  let choices = [correctAnswer];
  while (choices.length < 4) {
    let randomChoice = Math.floor(Math.random() * (selectedTable * 12)) + 1;
...
function gameLoop() {
  if (!gameOver && selectedTable !== null) { // Game updates only if table is selected
    updateGame();
  }
  drawGame();
