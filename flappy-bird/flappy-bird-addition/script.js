...
function startGame(maxNumber) {
  selectedMaxNumber = maxNumber; // Changed from table to maxNumber

  startScreen.style.display = 'none'; // Hide start screen
  generateAdditionProblem(); // Changed from generateMultiplicationProblem
  startMusic();
  preloadSounds();
...
function handleChoice(choice) {
  if (gameOver) return; // Ignore choices if game is over
  ...
  if (choice === currentProblem.answer) {
    ...
    wrongSound.play(); // Play dying sound for wrong answers
    endGame();
  }
}
...
function resetGame() {
  ...
  removeHighlight(); // Remove highlight from previous game
  startMusic(); // Restart music when restarting the game
}
...
function generateAdditionProblem() {
  if (selectedMaxNumber === null) {
    return; // Do not proceed if no max number is selected
  }

  // Generate two random numbers within the selected range
  const num1 = selectedMaxNumber;
  ...
  while (choices.length < 4) {
    // Generate plausible wrong answers around the correct answer
    let offset = Math.floor(Math.random() * 5) - 2; // -2 to +2
    let randomChoice = correctAnswer + offset;
    
    // Ensure the answer is positive and not already in choices
    if (randomChoice > 0 && !choices.includes(randomChoice)) {
  ...
  choices.forEach((choice, index) => {
    ...
    button.setAttribute('data-number', index + 1); // Add data attribute for number key
